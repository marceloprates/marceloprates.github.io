...
- JSON Resume files are **gitignored** since they're regenerated by
  the build pipeline (`scripts/generate-json-resume.ts`). Phase F
  decided to stop tracking them after observing that the LaTeX
  pipeline silently overwrote committed copies.
- PDFs are also gitignored build artifacts.
