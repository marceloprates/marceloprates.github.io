...
interface JsonResume {
  ...
  certifications: Array<{ title: string; date: string; issuer: string; url: string }>;
...
function buildNameLookup(macros: Record<string, string>): Record<string, string> {
...
function parseVariant(mainTex: string, macros: Record<string, string>) {
  ...
  const summaryRaw = extractMacroValue(mainTex, "\\renewcommand{\\ResumeSummary}");
  const summary = strip(summaryRaw);

  // Skills: \renewcommand{\ResumeCoreSkillsInline}{content}
  const skillsRaw = extractMacroValue(mainTex, "\\renewcommand{\\ResumeCoreSkillsInline}");
...
async function main() {
  ...
  const catalogPath = path.join(LOCAL_BASE, "includes", "data", "experience_catalog.tex");
