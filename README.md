...
src/                     # App code (App Router), components, lib, types, config
  ...
  data/                  # Build-time JSON data (committed where stable)
    resumes/             # ai.json, ai+ml.json, ds.json, ml.json (committed)
...
public/                  # Static assets served at site root
  ...
  resumes/               # Generated PDFs (committed)
...
- **Atomic commits**: one logical change per commit, conventional-commits
  ...
  `npm run precommit`.
- **Build artifacts**: `tsbuildinfo`, `.next`, `out`, and
  `public/search-index.json` are gitignored (regenerated by
  `npm run ci:check` / `npm run build:search-index`). Resume JSONs
  and PDFs are **committed** — see *Resume pipeline* below.
