...
const ghToken = process.env.GITHUB_TOKEN || process.env.GH_TOKEN || process.env.GITHUB_PAT || process.env.PERSONAL_TOKEN;
 ...
 */
export async function fetchRepo(repo: string): Promise<{ stars?: number; forks?: number }> {
  try {
 ...
 */
export async function fetchGitstar(repo: string): Promise<{ gitstarRank?: number; gitstarUrl?: string }> {
  try {
...
export async function enrichProjects(
  ...
  const enriched: Project[] = await Promise.all(
    rawProjects.map(async (p) => {
      ...
      return { ...p, repo, stars: stats.stars, forks: stats.forks, gitstarRank: gsr.gitstarRank, gitstarUrl: gsr.gitstarUrl } as Project;
    })
  );
  ...
  try {
    ...
    console.error('[project-enricher] Failed to write github-projects.json:', err);
  }
